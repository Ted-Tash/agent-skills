---
name: qa-handoff
description: Prepare test data and links for manual QA. Creates records in the correct states so the user can test the feature in the browser, then provides localhost URLs to the relevant pages. Use at the end of a feature implementation or when the user asks to set up test data.
---
...
Construct localhost URLs to the pages the user should visit. Determine the port from the project's docker compose config (e.g. `.dockerdev/compose.yml`).
...
**Test in browser:**
- <Page name>: <localhost URL>
- <Page name with filter>: <localhost URL with query params>
- ...
